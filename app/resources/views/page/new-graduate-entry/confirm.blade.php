...
@section('main-area')
    ...
    <form name="form" method="post" id="form" action="{{ route('new-graduate.complete') }}">
        @csrf
        ...
        @for ($i = 1; $i <= 4; $i++)
            ...
            <input name="educationals[{{ $i }}][month]" type="hidden"
                value="{{ $data['educationals'][$i]['month'] }}" />
            <input name="educationals[{{ $i }}][content]" type="hidden"
                value="{{ $data['educationals'][$i]['content'] }}" />
        @endfor
        ...
        <input name="personal_statement[focused]" type="hidden" value="{{ $data['personal_statement']['focused'] }}" />
        @for ($i = 1; $i <= 6; $i++)
        ...
        <div class="form-box clearfix">
            <h3>プロフィール情報</h3>
            <x-new-graduate-entry.profile-confirm :data="$data['profile']" />
        </div>

        <div class="form-box clearfix">
            <h3>住所情報</h3>
            <x-new-graduate-entry.address-confirm name="address" :data="$data['address']">
                <x-slot:foot>
                    <tr>
                        <th>連絡方法</th>
        ...
        <div class="form-box clearfix">
            ...
            @else
                <x-new-graduate-entry.address-confirm name="home" :data="$data['home']" />
            @endif
        </div>
        ...
        <div class="form-box clearfix">
            <h3>免許・資格</h3>
            <x-new-graduate-entry.resume-confirm count="5" name="licenses" :data="$data['licenses']" />
        </div>

        <div class="form-box clearfix">
            <h3>賞罰</h3>
            <x-new-graduate-entry.resume-confirm count="4" name="rewards" :data="$data['rewards']" />
        </div>
        ...
        <div class="form-box clearfix">
            <h3>身上書</h3>
            <x-new-graduate-entry.personal-statement-confirm :data="$data['personal_statement']" />
        </div>

        <div class="form-footer">
            <button type="submit" formaction="{{ route('new-graduate.back') }}">
                <span>入力内容を修正</span>
